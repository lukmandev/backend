...
const storage = multer.diskStorage({
  	destination: function (req, file, cb) {
   	 	cb(null, path.resolve(__dirname, process.env.UPLOAD_PATH));
  	},
...
router.post(
	...
	authMiddleware,
	[
		check('subtitle', 'Имя должно быть не меньше 10 и не больше 40').isLength({min: 2, max: 40}),
		check('courseName', 'Имя должно быть не меньше 10 и не больше 40').isLength({min: 2, max: 40}),
...
router.post(
	...
	[
		...
		check('subtitle', 'Имя должно быть не меньше 10 и не больше 40').exists(),
